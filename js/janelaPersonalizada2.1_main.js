...
const Janelas = function() {
					...
					if (typeof erro == "string") {
						...
						if (erro) {
							if (controle_alerta === null) {
								controle_alerta = that2.alerta(erro, "", function() {
									setTimeout(function() {
										that.manutencao.reabrir(controle_prompt);
										controle_prompt = null;
									}, 0);
								});
							} else controle_alerta.set.reabrir();
						} else {
							callback(salvarPorValor({
								prompt : valor
							}).prompt);
	...
	const Alterar = function() {
		...
		this.botaoOnOff = function(id, btn, ativar) {
			if (validarJanela(id, "manipular o botão de uma", false, true)) {
				try {
					const conteudo = that.lista[id].objeto.conteudo;
		...
		this.fechar = function(id) {
			...
			if (id == that.recursos.obterUltimo("ativo") && permissaoMouse) {
				...
				setTimeout(function() {
					permissaoMouse = true;
					if (callback) {
						const fn = that.lista[id].objeto.conteudo.cabecalho.botoes.fechar;
						if (typeof fn == "function") fn();
						Array.from(document.querySelectorAll(".menuRes > main, .menuRes > footer")).forEach((el) => {
	...
	const Recursos = function() {
		...
		this.zIndex = function(id) {
			...
			if (id === undefined) {
				for (x in that.lista) {
					if (that.lista[x].zIndex > retorno) retorno = that.lista[x].zIndex;
				}
			} else if (validarJanela(id, "conhecer informações de", false, false)) retorno = that.lista[id].zIndex;
	...
	const Controle = function() {
		...
		this.click = function(e) {
			...
			if (JP_mult) {
				...
				for (x in that.lista) {
					if (that.lista[x].aberto) abertos.push(x);
				}
				abertos.forEach((jan) => {
			...
			if (retorno) {
				...
				if (el !== null) retorno = chamaFechar(!el.contains(event.target), "mouse");
			}
			return retorno;
		}
		...
		this.keyup = function(e) {
			if (e.keyCode == 27) {
				...
				return chamaFechar(true, "teclado");
			}
			return false;
		}
		...
		this.keydown = function(e) {
			...
			if (!cond) {
				const ultimoAtivo = that.recursos.obterUltimo("ativo");
				let lUltimoAtivo = that.lista[ultimoAtivo];
		...
		this.resize = function() {
			...
			const dim = dimensoes();
			for (x in that.lista) {
				let ref = that.lista[x].dataEstilo;
		...
		this.executarFuncao = function(indice, indice2) {
			...
			if (that.recursos.obterUltimo("ativo") == id) {
				const botao = obterBotao(that.lista[id].objeto.conteudo.botoes[indice2]).objeto;
	...
	let chamaFechar = function(cond, tipo) {
		const ultimoAtivo = that.recursos.obterUltimo("ativo");
		if (ultimoAtivo && cond) {
			if (that.lista[ultimoAtivo].objeto.config.fechar[tipo]) that.manutencao.fechar();
			else animar();
		}
	...
	let validarJanela = function(id, objetivo, numero, aberto) {
		...
		if (id !== undefined) {
			...
			if (typeof id == "string") {
				if (that.lista[id] !== undefined) {
					if (aberto) {
						if (that.lista[id].aberto) retorno = true;
	...
	let sublinhaBotao = function() {
		const ultimoAtivo = that.recursos.obterUltimo("ativo");
		const todos = that.lista[ultimoAtivo].objeto.conteudo.botoes;
	...
	const fecharMain = function(id) {
		that.lista[id].aberto = false;
		that.lista[id].ativo = false;
		that.lista[id].sel = -1;
		full.splice(full.indexOf(id), 1);
	...
	const fechaQuandoPaiInvisivel = function() {
		...
		let abertos = 0;
		for (x in that.lista) {
			if (that.lista[x].aberto) {
				if (pai_invisivel) {
					fecharMain(x);
					const fn = that.lista[x].objeto.conteudo.cabecalho.botoes.fechar;
					if (typeof fn == "function") fn();
					if (haMenu()) document.getElementById("menuCobrir").style.display = "none";
	...
	const ativarClasse = function(id) {
		...
		for (x in that.lista) {
			...
			that.lista[x].ativo = x == id;
			document.getElementById(x).classList.remove("active");
			if (x == id) {
				lista = document.querySelectorAll("#" + x + " footer button");
				for (let i = 0; i < lista.length; i++) lista[i].disabled = !obterBotao(that.lista[id].objeto.conteudo.botoes[i]).objeto.ativo;
			}
		}
		...
		for (x in that.lista) {
			...
			if (haMenu()) {
				...
				estilo.zIndex = val + 2;
				document.getElementById("menuRes").style.zIndex = 1;
			}
		}
	}
	...
	const ativar = function(id) {
		setTimeout(function() {
			id = idOuUltimo(id);
			if (id) {
				let aux = that.lista[id];
				delete that.lista[id];
				that.lista[id] = aux;
				ativarClasse(id);
			}	
		}, 50);
	...
	const ativarBotaoPadraoUltimo = function(espera) {
		setTimeout(function() {
			const ultimoAtivo = that.recursos.obterUltimo("ativo");
			if (ultimoAtivo) {
				const _padrao = that.lista[ultimoAtivo].objeto.conteudo.botaoPadrao;
				if (_padrao > -1) {
					limparBotao();
	...
	const cria_e_altera = function(main, id, posiciona) {
		...
		Array.from(document.querySelectorAll(".janela .title-bar")).forEach((barra) => {
			...
			if (that.lista[id].objeto.conteudo.cabecalho.botoes.maximizar) {
				barra.ondblclick = function() {
					if (that.lista[id].objeto.conteudo.cabecalho.botoes.maximizar) that.manutencao.maxmin(id);
				};
			}
		});
